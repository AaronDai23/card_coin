...
class GetUidDataRunnable extends BaseRunnable {
  GetUidDataRunnable();

  @override
  Future<ScanResponse<String>> run(
      BuildContext context, IsoDepReaderManager readerManager) async {
    var response = await readerManager.sendCommand(getHeader(), encrypt: false);
    return handleResponse(response);
  }

  @override
  Uint8List getHeader() {
    return Uint8List.fromList([0x80, 0x64, 0x00, 0x00]);
  }
  ...
  ScanResponse<String> handleResponse(CommandResponse response,
    ...
    if (response.isSuccess) {
      ...
      int uidLenght = data[1];
      String uid = String.fromCharCodes(data.sublist(2, 2 + uidLenght));
      //
      print("GetUidDataRunnable-uid:$uid");

      return ScanResponse(true, data: uid);
    } else {
