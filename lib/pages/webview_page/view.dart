      ...
      NavigationDelegate(
        onNavigationRequest: (NavigationRequest request) {
          ...
          if (uri != null && uri.scheme == 'wc') {
            dispatch(WebviewActionCreator.onParsedWalletUrl(uri));
        ...
        onWebResourceError: (WebResourceError error) {
          ...
          if (targetUrl.isNotEmpty &&
            ...
            final subElapsed = state.pageLoadStartTime != null
                ? DateTime.now()
                    .difference(state.pageLoadStartTime!)
                    .inMilliseconds
                : -1;
            print(
          ...
          final mainErrElapsed = state.pageLoadStartTime != null
              ? DateTime.now()
                  .difference(state.pageLoadStartTime!)
                  .inMilliseconds
              : -1;
          ...
          dispatch(WebviewActionCreator.onShowProgressIndicator(false));
          print(
              'WebResourceError:description:${error.description}, errorCode:${error.errorCode},url:${error.url}');
          showDialog(
        ...
        onProgress: (int progress) {
          ...
          state.progressStallTimer?.cancel();
          if (progress < 100) {
            state.progressStallTimer = Timer(const Duration(seconds: 4), () {
              state.progressStallTimer = null;
              dispatch(WebviewActionCreator.onShowProgressIndicator(false));
            });
          }
          ...
          if (progress % 10 == 0 || progress == 100) {
            final progElapsed = state.pageLoadStartTime != null
                ? DateTime.now()
                    .difference(state.pageLoadStartTime!)
                    .inMilliseconds
                : -1;
          ...
          dispatch(WebviewActionCreator.onUpdateProgress(progress / 100));
        },
        onPageStarted: (url) {
          state.pageLoadStartTime = DateTime.now();
          print('[WebviewTiming] onPageStarted url=$url');
          state.progressStallTimer?.cancel();
          state.progressStallTimer = null;
          state.pageLoadTimeoutTimer?.cancel();
          state.pageLoadTimeoutTimer = Timer(_kWebPageLoadTimeout, () {
            // 超时后静默隐藏进度条，页面继续后台加载。
            dispatch(WebviewActionCreator.onShowProgressIndicator(false));
          });
          dispatch(WebviewActionCreator.onUpdateProgress(0));
          dispatch(WebviewActionCreator.onShowProgressIndicator(true));
        },
        onPageFinished: (url) {
          final finishedElapsed = state.pageLoadStartTime != null
              ? DateTime.now()
                  .difference(state.pageLoadStartTime!)
                  .inMilliseconds
              : -1;
          ...
          if (state.callback != null) {
            controller.runJavaScript(state.callback!);
          }
    ...
    ..addJavaScriptChannel(
      ...
      onMessageReceived: (JavaScriptMessage message) {
        ...
        if (pageInfo.actionType == 'ACTIVITY') {
          ...
          var canPush =
              pageRoutes.pages.keys.contains(pageInfo.actionTarget);
          if (canPush) {
            ...
            showToast(state.languageResource!
                .getUnsupportActivity(pageInfo.actionTarget ?? ''));
          }
        } else {
          ...
          if (pageInfo.operation == 'OPEN') {
            Navigator.of(viewService.context)
                .pushNamed('webviewPage', arguments: arguments);
          } else {
            Navigator.of(viewService.context)
